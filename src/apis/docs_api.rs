...
pub async fn download_table_schema(configuration: &configuration::Configuration, doc_id: &str, table_id: &str, header: Option<&str>) -> Result<models::TableSchemaResult, Error<DownloadTableSchemaError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::TableSchemaResult`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::TableSchemaResult`")))),
        }
    } else {
        let content = resp.text().await?;
